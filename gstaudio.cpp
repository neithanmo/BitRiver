...
gstaudio::gstaudio()
{
    ...
    if (!this->audiosrc || !this->asink || !this->conv || !bin || !volume){
    ...
    gst_bin_add_many(GST_BIN(bin), this->audiosrc, this->conv, volume, this->asink, NULL);
    gst_element_link_many(this->audiosrc, this->conv, volume, this->asink, NULL);
